...
## 当前结论（必须最新）
- 现状：采集脚本已具备“本页相关域名（含 iframe 插件域名）一键上传 -> Worker -> GitHub Actions -> 自动 PR”的可用链路，并已切换为“当前会话采集 + 云端 merge 去重”（废弃本地导出游标与采集仓持久化）。
- 已完成：采集脚本 UI/上传入口改造；Worker 新增 `POST /collector/submissions`（邀请码校验 + 直接 `repository_dispatch`，无 R2 临时存储）；新增采集 PR 工作流与 `collector-sanitize` 过滤器；Worker 已部署并完成真实上传联调（自动触发 Actions 并创建 PR）；一键上传默认范围已改为“本页相关域名”；超大请求已支持按字节自动分批上传；上传请求已加入 `GM_xmlhttpRequest` 优先通道以缓解插件页 `Failed to fetch`；采集器已废弃本地 `exportedTexts` 游标与采集仓持久化。
- 正在做：跟踪协作者真实使用反馈，观察多域名页面、超大批量页面、插件 iframe 页面下的稳定性与可理解性。
- 下一步：如仍频繁出现“误采到无关站点/域名太多导致批次偏大”，优先追加“上传前域名筛选 + 忽略域名名单（本地设置）”而不是逐条编辑器。
