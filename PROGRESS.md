...
- 已完成：已确认 ParaTranz 术语接口能力（`getTerms` / `createTerm` / `importTerms`）；已确认 `5etool.csv` 为 `GB18030` 编码、无表头、固定 5 列；已发现 CSV 内部存在重复术语（含同词不同译冲突），需要导入前去重与冲突报告。
- 正在做：实现并运行“CSV 术语增量导入脚本”（本地差集模式：先拉取远端术语，再仅上传缺失项）。
