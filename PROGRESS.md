...
- 正在做：Smoke & Spectre 5.x 翻译采集链正在从人工 Runtime Collector 迁移为 `manifest/static harvester + OBR Mock Host + Playwright + runtime fallback`。远端实验分支 `feat/obr-harvester` 已真实跑通 Smoke 生产 bundle 的静态资源发现与假 Owlbear 宿主启动；2026-09-10 已把该分支放入 `D:\Workspace\DnD5e\OBR2\OverlayLex-harvester` 独立 worktree，准备在 Windows 本机复跑与继续通用化。
- 下一步：① 本机复跑静态 Harvester 并核对生产 manifest/资源图；② 本机 Playwright 复跑 OBR Mock Host，确认 SDK request/response、自动导航、DOM/ARIA 采样在 Windows 同样稳定；③ 合并 i18n catalog + SDK registration + Mock DOM 并量化高置信覆盖；④ 将 Mock 协议层从 Smoke 专用脚本抽成可复用的 Owlbear Extension E2E harness；⑤ 旧 Collector 仅保留为运行时补漏/基准集工具。
...
- 决策AA：首发商店隐私政策如实披露“翻译包请求可间接表明当前支持域/插件”，但明确页面正文不上传；隐私政策先使用公开 GitHub 仓库稳定 URL，运行时翻译数据继续全部从自有 `overlaylex-api.zjhstudio.com` 获取。
- 决策AB：Firefox `all_frames` 最终回归使用临时 Firefox profile 的本地域 DNS 映射 + 固定跨域夹具，而不是依赖第三方插件公网首页能否脱离 Owlbear 上下文渲染（原因：这样测试真实 manifest host 权限和跨域 content script，同时排除第三方页面自身 CSP/初始化条件造成的假阴性）。
