...
## 当前结论（必须最新）
- 现状：已按最新设计回退启动逻辑为“缓存 allowlist 判准优先，seed 仅首装兜底”，并将云端更新统一后置到脚本启动完成后。
- 已完成：自定义域部署（`overlaylex-api.zjhstudio.com`）；主脚本失败提示；启动门禁重构到本地规则；脚本版本升级到 `0.2.15`。
- 正在做：等待用户侧验证“首装 + 无缓存 + seed 命中 / seed 未命中 / 有缓存 allowlist”三条启动路径。
- 下一步：如验证通过，按 `0.2.15` 发布；如仍有误判，再仅调整门禁状态机，不扩大 seeds 口径。
